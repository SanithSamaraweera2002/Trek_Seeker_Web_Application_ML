...
#         print(f"Destination: {destination}, Travel Time: {travel_time} minutes")
...
#     current_start = start
#     remaining_destinations = set(filtered_destinations)  # Use a set for fast membership checking

#     while remaining_destinations and total_time < day_duration:
#         # Run Dijkstra's algorithm to get the shortest paths from current_start
#         distances = dijkstra(distance_matrix, current_start, remaining_destinations)
        
#         # Sort destinations by travel time and get the top destination
...
#         top_destination, travel_time = sorted_destinations[0]
        
#         # Check if the top destination is valid and not already in the itinerary
#         if top_destination in visit_times and top_destination not in [dest for dest, _, _ in itinerary]:
#             visit_time = visit_times[top_destination]
#             travel_time += buffer_time
#             if total_time + travel_time + visit_time > day_duration:
#                 break

#             # Add the top destination to the itinerary
#             itinerary.append((top_destination, travel_time, visit_time))
#             total_time += travel_time + visit_time

#             # Update the current start to this top destination
#             current_start = top_destination
#             remaining_destinations.remove(top_destination)
#         else:
#             # If the top destination is not valid, remove it from the list and continue
#             remaining_destinations.discard(top_destination)
...
#     current_start = start  # The start point for the first iteration

#     for _ in range(len(filtered_destinations)):  # Iterate over all destinations
...
#         # Find the nearest valid destination that is not already in the itinerary
...
#             # No more valid destinations to visit
#             break

#         visit_time = visit_times[top_destination]
#         # travel_time += buffer_time  # Add buffer time for breaks or delays
        
#         if total_time + travel_time + visit_time > day_duration:
#             # If adding this destination exceeds the day's time limit, stop
#             break
...
#         # Update the current start to this new destination for the next iteration
#         current_start = top_destination

#         # Remove the selected destination from the filtered destinations
#         filtered_destinations.remove(top_destination)
...
#         if leftover_destinations:
#             filtered_destinations = leftover_destinations + filtered_destinations
...
#         print("LEFTOVER", leftover_destinations)
...
#         #     break
#         if not visit_times:  # If there are no more destinations to visit, stop
#             break       
...
def create_multi_day_itinerary(filtered_destinations, distance_matrix, start, visit_times, days, day_duration=600):
    ...
    remaining_destinations = visit_times.keys()
    last_destination = start
    leftover_destinations = []  # Track leftover destinations from the previous day

    for day in range(1, days + 1):
        # Add leftover destinations from the previous day to the start of filtered_destinations
        # if leftover_destinations:
        #     filtered_destinations = leftover_destinations + filtered_destinations
        #     leftover_destinations = []

        # Create the itinerary for the current day
        itinerary = create_itinerary(filtered_destinations, distance_matrix, last_destination, visit_times, day_duration)
        ...
        # Handle leftover destinations
        leftover_destinations = itinerary[4:]  # Store destinations not included in this day

        # Update the starting point for the next day's itinerary
        last_destination = leftover_destinations[0][0] if leftover_destinations else itinerary[-1][0]

         # Collect destinations to be removed (those visited today)
        visited_today = [destination for destination, _, _ in itinerary[:4]]

        # Safely remove only the visited destinations from visit_times
        visit_times = {key: val for key, val in visit_times.items() if key not in visited_today}

        # Also remove visited destinations from filtered_destinations
        filtered_destinations = [dest for dest in filtered_destinations if dest not in visited_today]

        # Remove destinations that have been visited from visit_times
        # for destination, _, _ in itinerary[:4]:
        #     if destination in visit_times:
        #         del visit_times[destination]
...
def generate_itinerary(recommendations, city, duration):
    ...
    print("DESTINATIONS FILTERED", filtered_destinations)
    # Visits time dict
